...
The system detects that something is wrong with the picture, so it decides not to save the report.
...
Jason is on a run and notices a car parked in the bike lane so he takes a picture and submits a report on the go. After analysis the submission, the system cannot determine the license plate number with enough accuracy, so it sends the photo to user review.
...
Once enough people review Jason’s photo, the system acquires enough votes to determine the license plate with good accuracy and marks the report as valid, making it available to all users.
