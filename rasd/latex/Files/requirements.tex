...
\item \textbf{\goalVisualizeReports}
    \begin{itemize}[label={}]
        \item {[FR4]} - The user is provided a map where the location of reports are indicated by markers.
        \item {[FR5]} - The user can filter the reports shown on the map by date, time and type of violation.
        \item {[FR6]} - The user can search for a specific location on the map by inputting coordinates or an address.
        \item {[FR7]} - The user can select a report on the map and obtain information about it. This information includes:
        \begin{itemize}[label={\textbullet}]
...
\item \textbf{\goalQueryInfo}
    \begin{itemize}[label={}]
        \item {[FR8]} - A secured API is provided to obtain and filter report information. The following filters are possible:
        \begin{itemize}[label={\textbullet}]
            \item Date and time
            \item Location
            \item Type of violation
        \end{itemize}
        \item {[FR9]} - The user can register by inputting his full name and email, and choosing a username and password.
        \item {[FR10]} - The user has access to view his full account information.
        \item {[FR11]} -The user can edit his email and full name.
        \item {[FR12]} - An API key is provided to the user via their profile screen in the mobile application.
        \item {[FR13]} - The user can login to the mobile application by providing their username and password.
    \end{itemize}

\item \textbf{\goalPhotosAuthoritiesOnly}
    \begin{itemize}[label={}]
        \item {[FR14]} - A secured API accessible only to authorities, provides the following information about reports:
        \begin{itemize}[label={\textbullet}]
            \item Photos
            \item License plate
        \end{itemize}
...
\item \textbf{\goalCuratedReports}
    \begin{itemize}[label={}]
        ...
        \item {[FR15]} - Detected cars and license plates are cross referenced with the license plate registry to ensure the license plate belongs to the car.
        \item {[FR16]} - A degree of confidence is assigned to each report, where a high confidence report will have:
        \begin{itemize}[label={\textbullet}]
        ...
        \item {[FR17]} - The API includes the system’s degree of confidence in the report to the provided report information after a query.
        \item {[FR18]} - Report information provided by the API can be filtered by their degree of confidence.
        \item {[FR19]} - Users can review photos with a license plate recognition lower than 80\% to adjust the confidence in the recognition.
    \end{itemize}
...
\item \textbf{\goalTicketAnalysis}
    \begin{itemize}[label={}]
        ...
        \item {[FR20]} - Information obtained from the ticketing system is used to determine if a report contributed to the issuing of a traffic ticket.
        \item {[FR21]} - Whether the report contributed to a traffic ticket is included in the information provided by the mobile application and the API.
    \end{itemize}
